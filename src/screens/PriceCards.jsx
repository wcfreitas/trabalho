...
import {
    ...
    ImageBackground, Image, TouchableOpacity, Animated
} from 'react-native';
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View style={style.containerCards}>
                                <Animated.Image style={[style.imageCard]}
                                    source={{ uri: item.card_images[0].image_url }} />
                                <View style={style.containerTextCards}>
                                    <Text style={style.textCards}>Cardmarket Price:
                                        ${item.card_prices[0].cardmarket_price}</Text>
                                    <Text style={style.textCards}>TCGPlayer Price:
                                        ${item.card_prices[0].tcgplayer_price}</Text>
                                    <Text style={style.textCards}>eBay Price:
                                        ${item.card_prices[0].ebay_price}</Text>
                                    <Text style={style.textCards}>Amazon Price:
                                        ${item.card_prices[0].amazon_price}</Text>
                                    <Text style={style.textCards}>Coolstuffinc Price:
                                        ${item.card_prices[0].coolstuffinc_price}</Text>
                                </View>
                            </View>
                        )}
                    />
